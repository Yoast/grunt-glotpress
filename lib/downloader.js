...
module.exports = {
	...
	get_api_url : function( options ) {
		return options.url + '/api/' + options.namespace + '/' + options.slug;
	},

	get_download_url : function( options ) {
		return options.url + ( options.downloadsUseApi ? '/api/' : '/' ) + options.namespace + '/' + options.slug;
	},
	...
	get_project_data : async function( options ) {
		try {
			...
			const translation_sets = data.translation_sets.filter( set => {
				...
				const { minimum_percentage, maximum_percentage } = options.filter;

				if ( minimum_percentage && percent_translated < minimum_percentage ) {
					return false;
				}

				if ( maximum_percentage && percent_translated > maximum_percentage ) {
					return false;
				}

                return set.word_counts.current > 0;
			} );
	...
	download_translations_from_set : async function( set, format, options ) {
		let url = this.get_download_url( options ) + '/' + set.locale + '/' + set.slug + '/export-translations?format=' + format;
